...
class TarifaService {
     ...
     * Se for casal (2 pessoas), aplica desconto de 10-15% em relação ao valor de 4 pessoas
     */
    static async _calcularPessoasExtras(chaleId, numAdultos, regras, numDiarias) {
        ...
        const limiteBase = regraPessoaExtra.limite_pessoas_base || 2;
        const pessoasExtras = Math.max(0, numAdultos - limiteBase);
        
        // Se for casal (2 pessoas) e o limite base for 2, aplicar desconto de 10-15%
        // O desconto será aplicado no cálculo do preço base, não aqui
        // Este método apenas calcula o adicional para pessoas acima do limite
    ...
    /**
     * Calcula desconto para casal (10-15% abaixo do valor de 4 pessoas)
     * @param {number} valorBase4Pessoas - Valor base para 4 pessoas
     * @param {number} numAdultos - Número de adultos
     * @returns {number} - Desconto a ser aplicado (valor negativo)
     */
    static _calcularDescontoCasal(valorBase4Pessoas, numAdultos) {
        // Se for casal (2 pessoas), aplicar desconto de 12.5% (média entre 10-15%)
        if (numAdultos === 2) {
            const percentualDesconto = 12.5; // Média entre 10% e 15%
            return -parseFloat((valorBase4Pessoas * (percentualDesconto / 100)).toFixed(2));
        }
        return 0;
    }
